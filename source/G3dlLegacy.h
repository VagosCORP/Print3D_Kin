...
void g3_um(float xa, float ya, float xb, float yb, float i, float j, float feed) {
    ...
    float sumNdt = (float)((float)dAngT*2.0)/dAng;
    ...
    char state_S2 = 1;
    int contState = 0;
    float cosAp = 0;//(float)1 - (float)(((float)dl*dl) / ((float)2*rc));//cos ang por delta (mágia rampas)
    float Z = 0;
    float valxi = xa;
    float valyi = ya;
    float valx = xa;
